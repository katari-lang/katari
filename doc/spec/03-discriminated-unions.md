...
ObjectFields(fields_L) - ObjectFields(fields_R):
  -- R にのみあるフィールドは無視 (パターンに書かれていないフィールドは他フィールド無視)
  -- L にのみあるフィールドはそのまま残す
  -- 共通フィールドについて、型を再帰的に引き算
  -- 共通フィールドの引き算結果のうち、いずれか一つでも never になったら
     ObjectFields 全体を absent (= never) として扱う
```

**オブジェクトパターンの他フィールド無視**:

パターン `{x = p}` は「フィールド `x` があり、`p` にマッチする」という意味であり、他のフィールドの有無・内容は問わない。パターン型は `{x: type(p)}` のみ（他フィールドなし）となり、型引き算でも他フィールドは無視される。

例:
```
型 {x: integer, y: string} | string
パターン {x = integer(n)}  → パターン型: {x: integer}

引き算: ({x: integer, y: string} | string) - {x: integer}
  → objectKind: {x: integer, y: string} から {x: integer} を引く
      x フィールド: integer - integer = absent (never)
      → x が never になったので object 全体 absent
  → string 部分はそのまま残る
結果: string
```
...
case パターン内の変数には型が推論される。推論は以下のルールに従う。

### DISC パターンの変数

object パターン内にちょうど 1 つの `uniq` フィールドがリテラルパターンとして存在する場合（DISC パターン）、残りのフィールド変数の型は DISC の該当バリアントから取得する。
...
task area(s: Shape) -> number {
  match s {
    case {uniq kind = "circle", radius = r} => {
      // r: number  (Shape DISC の "circle" バリアントの radius フィールド型)
      3.14 * r * r
    }
...
DISC パターン変数の型推論ルール:
1. 被マッチ式の型が DISC である場合、discriminator の値からバリアントを特定する
2. 特定したバリアントの NormalFields から、対応するフィールドの型を取得する
3. バリアントが存在しない場合（パターン側の discriminator 値が型にない）、変数型は `never`

### 通常 object パターンの変数

`uniq` なし、または複数の discriminator の食い違いがある場合は、通常 object パターンとして扱う。変数の型は、被マッチ式の型のうちそのフィールドを持つ部分の union から取得する。

```katari
task f(v: {x: integer, y: string} | {x: number, z: boolean}) -> integer | number {
  match v {
    case {x = x_val} => {
      // x_val: integer | number  (両バリアントの x フィールド型の union)
      x_val
    }
  }
}
...
```katari
case x: integer => { /* x: integer */ }
```

### 変数パターン

```katari
case x => { /* x: 残り型（被マッチ式の型から前のケースを引いたもの）*/ }
```

変数パターン `x`（型注釈なし）では、そのケースに到達した時点での残り型が `x` の型になる。
