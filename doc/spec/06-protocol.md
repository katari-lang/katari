...
agent の設計図。KATARI ソースコードの `agent` 宣言 (`AgentDecl`) に対応し、コンパイル後は IR の `IRAgentDef` となる。プロトコル上では `agent_def_id` で識別される。サーバーが提供する呼び出し可能な機能を定義する。

> **用語の対応**: ソースコードでは `AgentDecl` (宣言)、IR では `IRAgentDef` (定義)、プロトコル上では `agent_def_id` (定義の識別子) を使う。

### Agent (エージェント)

agent definition の実行インスタンス。固有の `agent_id` を持つ。agent は木構造 (親子関係) を形成する。ユーザーもネットワーク上の agent として扱われる。

> **`agent_def_id` と `agent_id` の区別**: `agent_def_id` は設計図 (定義) の識別子であり、同じ定義から複数のインスタンスを生成できる。`agent_id` は各インスタンスに付与される一意な識別子である。
...
| 操作 | 説明 |
| --- | --- |
| `spawn(agent_def_id, args, with_effects)` -> `agent_id` | 子 agent を生成する |
...
提供する agent definition (エージェント定義) の一覧を返す。
...
- `module_name`: モジュール名で絞り込む。exact match のみ (部分指定不可)。省略時は全 agent definition を返す。
...
- `agent_def_id`: agent definition の一意な識別子。
- `agent_def_where`: agent definition が位置する URL。POST /agent する際の宛先。
- `name`: agent definition の名前。モジュールがネストする場合は `path.to.agent_name` 形式。
- `description`: semantic annotation から生成された説明。
- `arg_types`: 引数の型の JSON Schema の配列。
- `return_type`: 返り値の型の JSON Schema。
- `with_effects`: この agent definition が発生させる可能性のある request 名のリスト。AI server がツール登録可否を判断する際に使用する。

**runtime サーバーの場合**: `external` 以外の全 agent definition を返す。
...
- `agent_id`: この agent インスタンスの一意な識別子。
- `agent_where`: この agent が位置する URL。
- `agent_def_id`: この agent の元となった agent definition の識別子。
...
- `agent_def_id`: 生成する agent の定義の識別子。
- `args`: agent に渡す引数。
- `parent_agent_id`: 親 agent の識別子。
- `parent_agent_where`: 結果を返すべき場所 (親 agent が位置する URL)。子 agent が request を発行する際の送信先にもなる。
- `with_effects`: この agent が発行できる request 名のリスト。型検証に使用する。
- `call_stack`: この agent が起動されるまでの agent definition の呼び出し経路 (最古 -> 最新順)。各エントリは `agent_def_id`、`agent_def_where` (起動サーバーの URL)、`agent_def_name` (デバッグ用の名前) で構成される。runtime が新たに spawn する際、自身の agent definition 情報を末尾に追加して渡す。AI server はこれを system prompt として AI に渡す (無限再帰防止)。
...
- `external_katari_endpoints`: 外部サーバー名と katari base URL のマッピング。`external agent` / `external request` の `from "server_name:..."` と対応。
...
- AI server はこの環境変数を参照して runtime の `GET /agent_def` から agent definition 一覧を取得し、ツール登録を行う。
- この値は docker-compose の `environment` セクション、または `.env` ファイルで設定する。

## 7. 冪等性

全ての request には `request_id` が付与される。`request_id` は冪等性キーとして機能し、同じ `request_id` を持つ request が複数回送信された場合、サーバーは重複処理を行わない。

これにより、ネットワーク障害等でリトライが発生した場合でも、request が二重に処理されることを防ぐ。サーバーは処理済みの `request_id` とその結果をキャッシュし、再送時は同一の結果を返す。
...
4. Break の値を agent の結果として返す
```

### par ブロックでの Effect 処理フロー
