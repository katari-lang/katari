...
//     `ctx.delegate(...)`. The tree view shows the ext call PLUS all
...
katari.agent("sleep_ms", async (ctx) => {
  const ms = ctx.args["ms"] as number;
...
katari.agent("fan_out", async (ctx) => {
  const callbackRaw = ctx.args["callback"];
  // The wire shape for a Katari callable arg is `{$agent: "qname"}`
  // (= per value-codec.ts). `ctx.delegate` expects the bare qname
  ...
  // child terminates propagate naturally via ctx.delegate's signal.
  const childSignal = ctx.signal;
  const children = Array.from({ length: count }, () =>
    ctx.delegate(callable as RawValue, {}, { signal: childSignal }),
  );
