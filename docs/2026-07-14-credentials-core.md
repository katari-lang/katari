...
1. **`oauth` reactor**(runtime): `token(name)` の external call。`resolveToken(store, name)` →
   `{token}` なら `string of private` を返して settle / `{needsAuthorize}` なら base の park 機構
   (`parkRequestName` override)で park、redispatch = 再 resolve。transient(throw)は typed
   `server_error` 系(oauth.ktr に throw を宣言)。reactor 追加の配線: `ReactorName` 型、
   `project-actor` の実体化、`Check.hs` の `externalReactorNames` + `stdlibOnlyReactorNames`。
2. **stdlib `prelude/oauth.ktr`**: `external agent token(name: string) -> string of private with io |
   prelude.throw[...] from "oauth"` + why-doc(「止まって聞く」契約、http.fetch との合成 idiom)。
   `katari-compiler.cabal` にファイル追加。`prelude.*` は予約済みなので K3022 整合は自動。
3. **configured client 登録**: 新テーブル `oauth_clients(project_id FK cascade, name) PK,
   issuer/authorize_endpoint/token_endpoint, client_id, client_secret(sealed, nullable=秘密なし public
   client), scopes(jsonb)` + module(PUT/GET(list, secret は write-only)/DELETE)。
4. **acquisition profile の解決規則(flow 開始時の直和)**: escalation argument に `url` があれば
   `mcp` profile(discovery+DCR)/ 無ければ `configured`(`oauth_clients[name]` を引く。無ければ 400
   「client 未登録」)。`oauth.token(name)` の park は `{name}` だけを argument に載せる。
5. **configured acquisition flow**: 登録 endpoint への plain OAuth 2.1 code+PKCE(client_secret があれば
   token 交換に併用)。既存の flow-state Map / public callback / deposit+answer-all をそのまま共用
   (profile はフロー開始の acquisition 分岐だけ)。取得した credential は `profile:"configured"` +
   tokenEndpoint(登録値)で保存 → core の refresh がそのまま効く。
6. **proactive login(flow 開始点の脱 escalation 化)**: pending flow の key を (project, name) に
   (escalationId を state から排除、再開始は置換)。新 endpoint
   `POST /projects/:id/credentials/:name/login`(mcp なら body `{url}`、configured なら body なし)→
   `{authorizationUrl}`。escalation 経由 endpoint は argument から (name, url?) を導出して同じ service を
   呼ぶ薄い層に。完了動作は一様(deposit + その name の open authorize escalation 全 answer、無ければ
   deposit のみ)。
7. **presentation / surfaces**: oauth presentation 変種は `{kind:"oauth", name, url: string | null}`
   (null = configured、server URL という概念が無い — 真の不在)。admin EscalationCard と CLI
   (Api.hs の `PresentationOauth` / Answer.hs)を追随。**Admin credentials ページ**(新): credentials
   一覧(name / updatedAt / profile)+ Log in / Re-authorize ボタン(popup-safe パターン再利用)+
   Forget + configured client の登録フォーム(secret は write-only)。CLI の proactive login は
   スコープ外(需要が出てから)。
8. **テスト**: oauth reactor(token 解決 / park→authorize→resume / transient throw)、configured flow
   (fake IdP、PKCE + client_secret 両方)、proactive login(escalation 無し deposit / open escalation
   全 answer)、flow 置換規則、client CRUD(secret write-only)。e2e は既存 13 green を維持
   (oauth.token の e2e 追加は fake IdP が要るため unit の live-loopback で代替)。
