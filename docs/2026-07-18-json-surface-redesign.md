...
- **無印の動詞は常に literal、wire 規約に触れるのは `encode` / `decode` / `to_text` の3語だけ**、
  という命名法則を仕様の中心に置く。`parse` / `stringify` / `parse_as` は literal に統一される
...
| **wire JSON** | 値↔文書の**全単射規約**(`$constructor` / `$ref` / `$agent` / `$closure` / `$tool` / `$$` エスケープ)。reactor 間・FFI・API・永続 envelope の形。SoT は `wire.ts` のまま不変 | `encode` / `decode` / `to_text` を自分で呼んだときだけ。それ以外で `$` を意識したら設計のバグ |
| **`json.json`** | **廃止** | — |
...
命名法則: **無印 = literal(文書をそのまま)。wire 規約は `encode` / `decode` / `to_text` の3語に
限局し、この3語は常に「値そのもの」を運ぶ。**「decode」の 2 義(wire 解釈 vs literal 解釈)は、
literal 側から decode という語を追放することで解消する。
...
| `json.stringify` | `(value: unknown) -> string` | **literal** 書き: 文書形(record / array / スカラー、blob 化 string 含む)をその文書テキストに。キーは書かれたまま | 文書形でない値(data / file / agent / closure / tool)は panic — 非有限数と同格の論理欠陥 |
...
| `json.to_text[T]` | `(value: T) -> string` | `stringify(encode(x))` の融合(encode の出力は常に文書形なので total)。エラー表示・handle 埋め込み・tool result 描画の万能テキスト化 — 今日と同義 | — |
...
- テキストレベルの wire 読みは合成で書ける: `decode[T](parse(text))` は `to_text` の逆
...
| 3 | `json.json` に file を置けない | **溶ける**: 文書=値ツリーなので file が置ける。意味は境界ごとに 1 契約 — `http.json` / mcp 引数は send 境界で base64、`stringify` は defect(panic)、`encode` は `$ref` handle 文書 |
| 4 | handle を送りたいときは明示 stringify | `json.encode(value = a_file)` を文書に埋める(または `to_text`)— 「wire form をくれ」という意図がそのまま動詞になる |
...
| 8 | `json_string` が blob 化 string を抱える | **残る・簡素化**: ラッパが消えて string 値そのもの。`stringify` / `to_text` は本文化、`decode` は保持 — 規約は同じで層が一つ減る |
...
| playground `tools.ktr` | `tool_list` → record リテラル(`input_schema = m.input` は代入のまま)。`parse_as` は綴り不変(意味は literal 化 — この文書に `$` キーはないので挙動同値)。`to_text` 不変 | 小 |
| playground `webhook.ktr` / `mcp_demo` / `basics` / discord-bot / katari-verify | `to_text` のみ — 不変 | なし |
