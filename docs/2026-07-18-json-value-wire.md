...
2. **`$$` エスケープ — 最終姿(2026-07-19 の json 表面再設計で決着)。** `$` 始まりの record キーは
   ...
   FFI port(decode)、http.json / mcp 引数の materializer(送信時)、`json.to_text`(表示時 —
   2026-07-19 に追加)。`json.parse` / `stringify` / `parse_as` は最初から literal(キー verbatim、
   escape 不関与)。新しい出口面を作るときは unescape を必ず入れること。`escapeRecordKey` は単射
