...
GET /api/v1/projects/{projectId}/snapshots/head
  200 → { id: string | null,                // snapshot id。未 deploy なら null
          message: string | null,
          modules: { [moduleName]: hash },   // 現 head が参照する per-module hash
          createdAt: string | null }
  // NOTE: snapshot の識別子は全エンドポイント (head / list / get / POST) で `id` に統一。
  //       list は modules を含めない軽量サマリ、get/head は manifest を含む。
