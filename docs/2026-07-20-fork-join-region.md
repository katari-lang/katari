...
(`files.from_base64` で "hello" を作り、fiber が `on_file(payload = it)` を perform、white hole の handler が
`files.read_base64` で読む):
...
補足の作法上の発見(その後 2026-07-20 に解消): `file` は予約 **型キーワード**なので、かつて module を
`prelude.file` と名付けていた頃は `file.gone` / `file.malformed_base64` を型位置で書けなかった(キーワード
`file` が module qualifier を shadow していた)。この gap を潰すため module を `prelude.files` に改名した — 型
キーワード `file`(値の型)はそのまま、qualifier だけを `files.` に。以後 `files.gone` / `files.malformed_base64`
は型位置でそのまま書け、error を catch する handler が `prelude.throw(error: files.gone)` と名指せる。
