...
| R7 | ~~single-server なので parent を実 FK で持つ~~ → **撤回: parent FK は持たず delegation 中心** (子は `delegation_id` のみ、親は `delegations.caller_instance_id` 経由で復元) | entity に parent を持たせず `D→E` map で復元 | delegation 行は ack correlation のため元々必要で caller を載せれば parent_instance_id は冗長 (§6 確定事項) |
...
**persist (load/checkpoint) の単位 (= shard)** でもある (closure は案2 で値なので所有物ではない)。
...
- **parent FK は持たない**。子は `delegation_id` のみを持ち、親は `delegations.caller_instance_id`
  経由で復元する (R7 撤回 → delegation 中心、§6)。`delegation_id` は自分の `delegateAck` の
  correlation も兼ねる (1 親が並列 delegate を複数走らせても区別できる)。
...
**案2 で確定** (§6 確定事項): closure を独立 entity にせず、値として
`{ blockId, scopeId, snapshot }` を直接持ちまわす。owned resource は **Scope だけ** に縮み、
ascent/GC は脱出値の中の scopeId を辿るだけ。`closures` table は無い。closure 値が block を解決する
ため snapshot を値に持つ。
...
### Delegation
親が子 instance を召喚した「**リクエスト辺**」の durable 記録 (発行元 = 親が管理)。**親→子の辺は
これが正本** — instance は parent FK を持たず、`delegations.caller_instance_id` が親を指す
(caller に cascade)。crash recovery 用の outbox であり、`delegateAck` の相関 id でもある (1 親が
並列に複数 delegate しても、どの delegate の返信かを子の `delegation_id` で区別)。state:
`running | cancelling`。ack 受信で発行元が削除。
...
Scope ──── Variable           (CASCADE; 実体は scope.values の値マップ。Closure は案2 で値なので非所有)
...
> **更新 (2026-07-19)**: **blob** の ascent は値駆動をやめ、上向きイベント(`delegateAck` /
> `escalate`)ごとに送信 instance の全 blob を 1 段上へ無条件 hoist する方式へ変更した(text 平面が
> id を運ぶが所有権を運ばないため)。scope の ascent(この節の reachability drag)は無変更。詳細と
> 境界条件は `docs/2026-07-19-ownership-hoist.md`。
...
instances(
  ...
  delegation_id→delegations (set null),  -- 召喚した delegation (null = project root)。親 FK は持たず、
                                          --   親は delegations.caller_instance_id 経由で復元。ack correlation も兼ねる
  ...
  status,                             -- running | cancelling のみ (terminal は持たない — runs が持つ)
  created_at, updated_at,
  INDEX(project_id))

-- ── engine graph (instance 所有, CASCADE) ──────────────────
threads(project_id, instance_id→instances ⌫, thread_id,
        kind, parent_thread_id, parent_call_id, scope_id, block_id, status,
        payload JSONB,                -- kind 別 variant state (葉)
...
scopes(project_id, scope_id, parent_scope_id,
       ...
       values JSONB,                  -- VariableId→Value の変数マップを inline (scope_variables は廃止)
       PK(project_id, scope_id), INDEX(owner_instance_id))
blobs(project_id, blob_id, owner_instance_id→instances ⌫,  -- owner NULL = in-transit (ascent)
       ...
       PK(project_id, blob_id), INDEX(owner_instance_id))   -- bytes は BlobStore (S3)、table は metadata のみ

-- ── request / capability 辺 ────────────────────────────────
delegations(id, project_id→projects ⌫, caller_instance_id→instances ⌫,  -- 親→子の辺 + ack correlation
       target JSONB, argument JSONB, state, created_at, updated_at,
       INDEX(caller_instance_id))
escalations(id, project_id→projects ⌫, raiser_instance_id→instances ⌫,
       request, argument JSONB, state, created_at)  -- request = 要求 capability の qname。state = open のみ

-- ── API 管理 ───────────────────────────────────────────────
runs(id, project_id→projects ⌫, instance_id→instances (set null), snapshot_id→snapshots,
       name, qualified_name, argument JSONB, state,   -- instance は terminal で self-delete するので nullable
       result JSONB, error_message, cancel_reason, created_at, updated_at, completed_at)
...
external_calls(id, project_id→projects ⌫, instance_id→instances ⌫,
       thread_id, key, argument JSONB, state, created_at)
```

### 確定事項 (実装で決定, 2026-06-16)

§6 の旧「検討事項」は実装で次のとおり確定した。

- **persist 粒度 = 行ごと 3NF** (threads / scopes を別行)。変数は scope に畳む (下記)。hot path の
  persist で重ければ instance 単位 JSONB に退避できる (schema 互換は気にしない)。
- **closure = 案2 で確定**: 独立 entity にせず値 `{ blockId, scopeId, snapshot }` で持ちまわす。
  owned resource は Scope に一本化、`closures` table は **無し**。
- **scope_variables は廃止 → `scopes.values JSONB`**: 変数は scope 単位でしか出入りせず Value は
  すでに JSON 葉なので、行分解しても関係構造が増えない (engine も 1 つの inline map で保持)。
- **delegation table は残す**: instance は parent FK を持たず `delegation_id` のみを持つので、
  delegations が **親→子の辺そのもの** (caller_instance_id, instance に cascade)。parallel delegate
  (FFI 並列など) の delegateAck を correlate する役目も担い、crash recovery の outbox も兼ねる。
- **blob bytes = S3** (BlobStore 抽象、v0.1.0 は S3 byte 列保存)。blob table は metadata
  (hash / size / content_type / semantic_kind) のみ、bytes 列は持たない。

> なお §0 R7「single-server なので parent を実 FK で持つ」は本実装で **撤回**した。parent FK は
> 持たず、上の delegation 中心モデル (子は delegation_id のみ、親は delegations.caller_instance_id
> 経由で復元) を採る。理由: delegation 行は ack correlation のため元々必要で、caller を載せれば
> parent_instance_id は冗長になるため。
