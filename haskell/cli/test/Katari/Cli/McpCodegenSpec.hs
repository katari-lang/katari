...
golden =
  Text.unlines
    [ "// Generated by `katari mcp pull --url https://mcp.example.test/mcp --out src/github.ktr`.",
      ...
      "",
      "@\"Open the pulled MCP server's connection for the extent of @continuation@ — as a bare `use github.connect(auth = ...)`, after which the tools are called directly (`github.get_issue(...)`). Opens a listing-free `mcp.open` scope over `https://mcp.example.test/mcp` (no server round-trip until the first tool call), serves the connection's `credentials` to every tool, and discharges both on return. `auth` is `mcp.headers(values = ...)` for header or anonymous access, or `mcp.oauth(name = \\\"...\\\")` for a server-stored credential (a missing one pauses the run on an OAuth authorization escalation; answer it from the admin console or `katari answer`).\"",
      "agent connect[R, effect E](",
      "  auth: mcp.auth,",
      "  continuation: agent (value: null) -> R with {...(E | connection), credentials},",
      ") -> R with io | E {",
      "  use mcp.open[connection, R, E](url = \"https://mcp.example.test/mcp\", auth = auth)",
      "  use handler {",
