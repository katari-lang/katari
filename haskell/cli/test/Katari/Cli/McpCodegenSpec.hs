...
golden =
  Text.unlines
    [ "// Generated by `katari mcp pull --url https://mcp.example.test/mcp --out src/github.ktr --name github`.",
      ...
      "",
      "@\"Connect to the pulled MCP server — returns the typed tools closed over the connection (@url@ +\\n@auth@). `auth` is `mcp.headers(values = ...)` for header or anonymous access, or\\n`mcp.oauth(name = \\\"github\\\")` for a credential established by `katari mcp login`.\"",
      "agent connect(url: string ?= \"https://mcp.example.test/mcp\", auth: mcp.auth) -> {",
      "  get_issue: agent(filter?: json.json, labels?: array[string], owner: string, repo: string) -> get_issue_output with io | prelude.throw[mcp.server_error | mcp.auth_error | json.decode_error],",
      "  ping: agent(arguments: json.json) -> json.json with io | prelude.throw[mcp.server_error | mcp.auth_error],",
      "} {",
      "",
      "  @\"Fetch one issue.\\nSlow on \\\"cold\\\" repos.\" agent get_issue(filter: json.json | null ?= null, labels: array[string] | null ?= null, owner: string, repo: string) -> get_issue_output with io | prelude.throw[mcp.server_error | mcp.auth_error | json.decode_error] {",
      ...
      "    let raw = mcp.call(url = url, auth = auth, tool = \"get-issue\", arguments = json.json_object(entries = arguments_4))",
      "    json.decode[get_issue_output](value = raw)",
      "  }",
      "",
      "  agent ping(arguments: json.json) -> json.json with io | prelude.throw[mcp.server_error | mcp.auth_error] {",
      "    let raw = mcp.call(url = url, auth = auth, tool = \"ping\", arguments = arguments)",
      "    raw",
      "  }",
      "",
      "  {",
