...
printIRModule irm = T.intercalate "\n"
  [ "module: " <> irmName irm
  , ""
  , "=== consts ==="
...
showConst CVNull      = "null"
showConst (CVBool b)  = if b then "true" else "false"
showConst (CVInt  i)  = T.pack (show i)
showConst (CVNum  d)  = T.pack (show d)
showConst (CVStr  s)  = "\"" <> s <> "\""
...
printRequests rs = T.unlines (map go rs)
  where
    go r = "  [" <> T.pack (show (irReqId r)) <> "] " <> irReqName r <>
...
printHandleBlock irm hb =
  [ "    handle[" <> showId (irhId hb) <>
    "] states=[" <> commaSep (map (vn nt) (irhStateVars hb)) <> "]"
  ] ++
  concatMap printReqCase (irhReqCases hb) ++
  case irhReturnCase hb of
    Nothing    -> []
    Just instrs ->
      "      return:" : printInstrs 8 irm instrs
  where
    nt = irmNameTable irm
    printReqCase (rid, instrs) =
      ("      request[" <> showId rid <> "] " <> rn nt rid <> ":") :
