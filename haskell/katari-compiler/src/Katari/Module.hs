...
foldM f acc xs = case xs of
  [] -> return acc
  x : rest -> f acc x >>= \acc' -> foldM f acc' rest
...
addDecl env decl = case decl of
  ...
  DeclImport _ _ -> return env -- imports handled at module load time
