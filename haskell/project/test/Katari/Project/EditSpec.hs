...
import Katari.Project.Edit (EditError (..), rewritePackages, rewriteSnapshot)
import Test.Hspec
...
spec = do
  ...
  describe "rewriteSnapshot" $ do
    it "replaces the pin and preserves every other byte" $ do
      rewritten <- expectRight (rewriteSnapshot sampleConfig "snapshot-2026-07-26-a7cc1e51")
      rewritten
        `shouldBe` Text.unlines
          [ "# my project",
            "[package]",
            ...
            "[dependencies]",
            "registry = \"https://example.com/registry\"",
            "snapshot = \"snapshot-2026-07-26-a7cc1e51\"",
            "packages = [\"list_utils\"]",
            "",
            "[runtime]",
            "url = \"http://localhost:3000\""
    ...
    it "preserves the assignment's own spacing style and a trailing comment" $ do
      let spaced = "[dependencies]\nsnapshot   =   \"staging\" # early access\n"
      rewritten <- expectRight (rewriteSnapshot spaced "snapshot-2026-07-26-a7cc1e51")
      rewritten `shouldBe` "[dependencies]\nsnapshot   =   \"snapshot-2026-07-26-a7cc1e51\" # early access\n"

    it "replaces a literal-string pin as readily as a basic one" $ do
      rewritten <- expectRight (rewriteSnapshot "[dependencies]\nsnapshot = 'staging'\n" "staging")
      rewritten `shouldBe` "[dependencies]\nsnapshot = \"staging\"\n"

    it "inserts the snapshot key right under a [dependencies] header lacking one" $ do
      let noKey = "[dependencies]\npackages = []\n"
      rewritten <- expectRight (rewriteSnapshot noKey "staging")
      rewritten `shouldBe` "[dependencies]\nsnapshot = \"staging\"\npackages = []\n"

    it "is idempotent: re-pinning to the same snapshot changes nothing further" $ do
      once <- expectRight (rewriteSnapshot sampleConfig "staging")
      once `shouldBe` sampleConfig

    it "does not mistake a longer key for snapshot" $ do
      let lookalike = "[dependencies]\nsnapshot_notes = \"z\"\nsnapshot = \"staging\"\n"
      rewritten <- expectRight (rewriteSnapshot lookalike "next")
      rewritten `shouldBe` "[dependencies]\nsnapshot_notes = \"z\"\nsnapshot = \"next\"\n"

    it "refuses a pin that would need escaping to splice in" $
      rewriteSnapshot sampleConfig "not a snapshot" `shouldSatisfy` either (const True) (const False)

    it "refuses a value that is not a single-line quoted string" $
      rewriteSnapshot "[dependencies]\nsnapshot = \"\"\"staging\n" "next"
