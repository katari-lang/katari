...
-- katari_compiler = "0.1.0"       # frozen from the snapshot (omitted when none was loaded)
--
-- [packages.list_utils]
-- source = "git"
...
-- @
--
-- The lockfile is generated by @katari lock@ (and by @katari add@ / @remove@ / @update@, which
-- reconcile it in the same breath as the manifest edit they make) and is meant to be committed so
-- every consumer gets the same byte-for-byte resolution. Nothing else writes it: @check@, @build@
-- and @apply@ only read it, and refuse when it disagrees with @katari.toml@.
--
...
data Lockfile = Lockfile
  { -- | Schema version (see 'lockfileFormatVersion').
    ...
    snapshot :: Maybe Text,
    -- | The @katari_compiler@ the snapshot declared, frozen alongside its packages. Recorded so an
    -- offline command can still warn that the locked set targets a different compiler — the snapshot
    -- itself lives on the network, and re-reading it is exactly what an offline load must not do.
    -- 'Nothing' when resolution never loaded a snapshot, and on any lock written before this field
    -- existed (an absent optional key reads back as "unknown", not as a parse failure).
...
keyVersion, keySnapshot, keyKatariCompiler, keySource, keyUrl, keyRev, keySha256, keyPath :: Text
keyVersion = "version"
keySnapshot = "snapshot"
keyKatariCompiler = "katari_compiler"
keySource = "source"
keyUrl = "url"
keyRev = "rev"
keySha256 = "sha256"
...
instance DecodeTOML RawLockfile where
  tomlDecoder =
    RawLockfile
      ...
      <*> getFieldsOpt [tableLock, keyKatariCompiler]
    ...
    pure
      Lockfile
        { version = raw.version,
          ...
          katariCompiler = raw.katariCompiler,
          packages = Map.fromList validatedPackages
        }
...
renderLockfile lockfile =
  Text.unlines $
    [tableHeader tableLock, assign keyVersion (Text.pack (show lockfile.version))]
      ...
      <> maybe [] (\compilerValue -> [assign keyKatariCompiler (quote compilerValue)]) lockfile.katariCompiler
