-- | Entry point for the Katari LSP server. Constructs the
-- 'ServerState' once at startup and threads it through every
-- per-method handler.
  ...
  LSP.runServer $
    LSP.ServerDefinition
      { LSP.defaultConfig = (),
        ...
        LSP.staticHandlers = \_clientCaps ->
          mconcat
            [ documentHandlers st,
              hoverHandler st,
              ...
              -- Silently absorb optional LSP lifecycle / trace
              -- notifications so the framework does not log them as
              -- "no handler". v1 does not act on watched-file disk
              -- events or didSave (didChange already triggers a
              -- debounced recompile on every keystroke).
        ...
        LSP.options =
          LSP.defaultOptions
            { LSP.optTextDocumentSync = Just textSyncOptions,
              -- Auto-trigger completion after `.` (member / field
              -- access) and `(` (call-argument label). Without these
              -- the LSP client only requests completion on explicit
              -- user invocation (= Ctrl-Space).
