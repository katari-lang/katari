...
One project, nineteen modules, each independently runnable. Use it to smoke-test a runtime and to see
...
| [`mailbox.ktr`](src/playground/mailbox.ktr)         | `playground.mailbox.main`     | CROSS-NURSERY MAIL — the composition several long-lived residents are built from: each resident fiber opens a nursery of its OWN (its mailbox), announces the handle with a `register` request, and blocks on `region.watch`; a desk holding those handles delivers by forking a NAMED task with PLAIN DATA into one (`region.fork` routes by the handle alone, so the letter surfaces in the addressee's conversation — a request could not have reached it). Also: two letters into one mailbox are read in posting order, and a fork into a dismissed resident's closed nursery panics, folded to a value with `supervise.once` + `signal_panics` |
...
katari run playground.mailbox.main                       # => core heard … | core heard … | scribe heard … | core dismissed | the letter to core bounced: …
