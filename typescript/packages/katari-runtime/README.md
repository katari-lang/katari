...
- `statementMakeClosure` は新 `ClosureId` を発行し、`state.closures[closureId] = { blockId, scopeId: thread.scopeId }` に登録、Value として `{ kind: "closure", closureId }` を bind する。
- `statementCall callTargetValue` で closure を呼ぶと `state.closures[value.closureId]` から `(blockId, scopeId)` を引き、子 thread を作る際にその scope を inherit する。
- GC は scope と同列に closures map も掃除する。closure value から到達不能になった entry は `state.closures` から削除される。
