 ...
 */
export function useSnapshotMessage(projectId: string | undefined): {
  ...
  const snapshotsQ = useQuery({
    ...
    queryFn: () => client.listSnapshots(projectId as ProjectId, { limit: 200 }),
    enabled: typeof projectId === "string",
  });

  const messageById = new Map((snapshotsQ.data?.snapshots ?? []).map((s) => [s.id, s.message]));

  function getMessage(snapshotId: SnapshotId | undefined): string | undefined {
