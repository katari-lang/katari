...
// host: escalations are read from the Layer 1 repository, the completed `StoredCredential` is sealed with
...
export const mcpAuthorizationFlow = createMcpAuthorizationFlow({
  ...
  depositCredential: (projectId, name, credential) =>
    credentialRepository.upsert(db, projectId, name, encryptSecret(JSON.stringify(credential))),
