...
//
// Single-server (R7): an instance carries a real `parent_instance_id` FK, so the tree is a plain
// self-referential table. `target` holds the agent reference `(qname, snapshot) | closure`; `snapshotId`
// is the version denormalised out of it for the FK / RESTRICT (a running version cannot be deleted).
...
export const instances = pgTable("instances", {
  ...
  /** Parent in the execution tree; `null` for the project root. */
  parentInstanceId: uuid("parent_instance_id").references((): AnyPgColumn => instances.id, {
    onDelete: "cascade",
  }),
  /** The delegation that summoned this instance; `null` for the project root. */
...
/** Issuer-managed request edge: the parent's durable record of a child it summoned (recovery outbox). */
